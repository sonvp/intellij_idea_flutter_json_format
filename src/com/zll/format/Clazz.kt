...
abstract class Clazz(
    ...
    fun getStatement() = "${getClassName()}${if(Settings().generateNullSafety && getClassName() != "dynamic") "?" else ""} ${getCamelName()};"
...
data class ObjectClazz(
    ...
    override fun getClassName() = "${Util.toUpperCaseFirstOne(name.replace("_",""))}Bean"
    override fun getAssignments(parent: String) = listOf("$parent.${getCamelName()} = ${getClassName()}.fromJson(map['$name']);")
    override fun map(obj: String): String {
        return "${getClassName()}.fromJson($obj)"
    }
}
